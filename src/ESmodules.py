...
def GetES(Value, GeneSet):
    setIDX = set(GeneSet).intersection(Value.index)
...
def GetES_DF(Exp, gmtFile, core = 80):
...
''' Dataframe 너무 클 경우(>1000) 잘라서 하도록 새로 만듦
def GetES_DF(Exp, gmtFile, core = 80):
    Exp.index = [x.split(':')[0] for x in Exp.index]
    gmt_ = gm.LoadGMT2Dict(gmtFile)
    with mp.Pool(core) as pool:
...
def GetES(Value, GeneSet):
    #Value = exp_f1.iloc[:,i]
    #GeneSet = gmt_c6['ATF2_S_UP.V1_UP']
    Value.index = [x.split(':')[0] for x in Value.index]
    ks_G = ks_2samp(Value, Value[set(GeneSet).intersection(Value.index)], alternative='greater')
    ks_L = ks_2samp(Value, Value[set(GeneSet).intersection(Value.index)], alternative='less')
    if ks_G[1] < ks_L[1]:
        return ks_G[0] 
    else:
        return ks_L[0] * -1
...
def GES(ky, Exp, gmt_):
    ...
    for i in range(Exp.shape[1]):
        res.append(GetES(Exp.iloc[:, i], gmt_[ky]))
    return res
